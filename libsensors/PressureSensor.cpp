      ...
      mHasPendingEvent(false)
{
   ...
   // ALOGI("PressureSensor: input: %s",input_name);
}
...
int PressureSensor::setInitialState() {
    ...
    if (!ioctl(data_fd, EVIOCGABS(EVENT_TYPE_PRESSURE_W), &absinfo)) {
...
int PressureSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count && mInputReader.readEvent(&event)) {
        int type = event->type;
        if (type == EV_REL) {
            if (event->code == EVENT_TYPE_PRESSURE_H || 
	      event->code == EVENT_TYPE_PRESSURE_D ||
	      event->code == EVENT_TYPE_PRESSURE_W ) {
