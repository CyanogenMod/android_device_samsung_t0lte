      ...
      mHasPendingEvent(false)
{
    ...
    ALOGI("LightSensor: input: %s",input_name);
}
...
int LightSensor::setInitialState() {
    ...
    if (!ioctl(data_fd, EVIOCGABS(EVENT_TYPE_LIGHT_HW), &absinfo)) {
...
int LightSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count && mInputReader.readEvent(&event)) {
        int type = event->type;
        if (type == EV_REL) {
             if (event->code == EVENT_TYPE_LIGHT_HW ||
	      event->code == EVENT_TYPE_LIGHT_W ||
	      event->code == EVENT_TYPE_LIGHT_D ||
	      event->code == EVENT_TYPE_LIGHT_M) {
