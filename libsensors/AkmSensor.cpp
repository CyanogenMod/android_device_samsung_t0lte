      ...
      mInputReader(32)
{
	...
	//First try always seems to fail
	if(data_fd < 0){
	  ALOGI("AkmSensor: retrying to open compass sensor");
	  data_fd = openInput("compass_sensor");
	}
	
	if(data_fd > 0){
	  ALOGI("AkmSensor: compass sensor successfully opened: %i", data_fd);
	}else{
	  ALOGI("AkmSensor: failed to open compass sensor");
	}
    ...
    short flags = 0;
/*ALOGI("AkmSensor: Testing ACCELL");
    if (akm_is_sensor_enabled(SENSOR_TYPE_ACCELEROMETER))  {
      ALOGI("AkmSensor: Testing ACCELL - enabled");
...
{
    int what = -1;
   // ALOGI("AKmSensor: handle- %i / en: %i", handle, en);
    switch (handle) {
...
/*	    if (!akm_is_sensor_enabled(SENSOR_TYPE_MAGNETIC_FIELD) && en)  {
	        err = akm_enable_sensor(SENSOR_TYPE_MAGNETIC_FIELD);
	    }else if (akm_is_sensor_enabled(SENSOR_TYPE_MAGNETIC_FIELD) && !en){
	        err = akm_disable_sensor(SENSOR_TYPE_MAGNETIC_FIELD);
	    }*/
...
int AkmSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count && mInputReader.readEvent(&event)) {
        int type = event->type;
        if (type == EV_REL) {
	  //ALOGI("AkmSensor: EV_REL");
            processEvent(event->code, event->value);
            mInputReader.next();
        } else if (type == EV_ABS) {
	 // ALOGI("AkmSensor: EV_ABS");
            processEvent(event->code, event->value);
            mInputReader.next();
        } else if (type == EV_SYN) {
...
void AkmSensor::processEvent(int code, int value)
{
  ...
  //ALOGI("AkmSensor: processEvent - code: %i  value: %i", code, value);
    switch (code) {
