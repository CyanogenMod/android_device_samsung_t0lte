...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        if (csd_volume == NULL) {
            ALOGE("dlsym: Error:%s Loading csd_volume", dlerror());
        } else {
            ALOGD("%s: calling csd_volume, volume=%d", __func__, adev->voice_volume);
            csd_volume(adev->voice_volume);
        }
    }
    return 0;
}
