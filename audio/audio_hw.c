...
{  
    ...
    switch(adev->out_device) {
      case AUDIO_DEVICE_OUT_SPEAKER:
          ...
          ALOGV("%s: SPEAKER Boost Volume: %i", __func__, volume);
          mixer_ctl_set_value(adev->mixer_ctls.speaker_volume,0,DB_TO_SPEAKER_VOLUME(volume));
          mixer_ctl_set_value(adev->mixer_ctls.speaker_volume,1,DB_TO_SPEAKER_VOLUME(volume));
          break;
      case AUDIO_DEVICE_OUT_WIRED_HEADSET:
      case AUDIO_DEVICE_OUT_WIRED_HEADPHONE:
          volume = get_volume(OUT_HEADPHONE);
          ALOGV("%s: HEADPHONE Boost Volume: %i", __func__, volume);
          mixer_ctl_set_value(adev->mixer_ctls.headphone_volume,0,DB_TO_HEADPHONE_VOLUME(volume));
          mixer_ctl_set_value(adev->mixer_ctls.headphone_volume,1,DB_TO_HEADPHONE_VOLUME(volume));
          break;
    }
}
